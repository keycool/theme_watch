...
CACHE_DIR = Path(__file__).with_name(".cache_scan")
...
def fetch_with_cache(name: str, fetcher, dtype=None) -> pd.DataFrame:
    ...
    try:
        ...
        if "频率超限" in str(exc):
            raise RuntimeError(f"{name} 当前被限频，且本地还没有缓存，稍后重试。原始报错: {exc}")
        raise
...
def get_all_sw_daily(pro, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    numeric_cols = ["open", "high", "low", "close", "change", "pct_change", "vol", "amount", "pe", "pb", "float_mv", "total_mv"]
...
def get_stk_limit_all(pro, trade_date: str) -> pd.DataFrame:
    try:
        ...
        df = fetch_with_cache(
            f"stk_limit_fallback_{trade_date}",
            lambda: pro.stk_limit(ts_code="600519.SH", start_date=trade_date, end_date=trade_date),
        )
...
def run_scan(end_date: str = "20260630") -> pd.DataFrame:
    ...
    latest_date = dates[-1]
    prev_date = dates[-2]
    start_date = (pd.Timestamp(latest_date) - pd.Timedelta(days=420)).strftime("%Y%m%d")
